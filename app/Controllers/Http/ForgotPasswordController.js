...
class ForgotPasswordController {
  async store ({ request, response }) {
    try {
      ...
      await Mail.send(
        ['emails.forgot_password'], // ['emails.forgot_password', 'emails.forgot_password_text'], <- para não cair no spam, cria uma versão sem HTML
        { email, token: user.token, link: `${request.input('redirect_url')}?token${user.token}` },
        message => {
          message
            .to(user.email)
            .from('mauro@gmail.com', 'Mauro | Teste')
